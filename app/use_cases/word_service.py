...
Attributes:
    all_words (List[Word]): List of all words for the quiz.
    logger (QuizLogger): Logger for recording quiz results.
    correct (int): Number of correct answers.
    incorrect (int): Number of incorrect answers.
    incorrect_words (List[str]): List of incorrectly answered words.
    correct_words (List[str]): List of correctly answered words.
    start_time (datetime): Start time of the quiz.
    mode (QuizMode): Current quiz mode.
    word_queue (List[Word]): Queue of words for the quiz.
    current_word_index (int): Index of the current word in the queue.
    repeat_incorrect_count (dict): Counter for how many times each word was answered incorrectly.
...
class WordService:
    ...
    def __init__(self, words: List[Word], logger: QuizLogger):
        ...
        Initializes the WordService with a list of words and a logger.
        Args:
            words (List[Word]): A list of Word objects to be used in the quiz.
            logger (QuizLogger): A logger instance for logging quiz activities.
        """
    ...
    def end_quiz(self):
        ...
        Attributes:
            ...
            correct_words (list): The list of correctly answered words.
            incorrect_words (list): The list of incorrectly answered words.
            start_time (datetime): The start time of the quiz session.
            logger (Logger): The logger instance used to log the quiz results.
        """
    ...
    def get_next_word(self) -> Optional[Word]:
        ...
        Returns:
            Optional[Word]: The next word to be used in the quiz. Returns a Word object if a word is available,
    ...
    def _get_next_word_normal(self) -> Optional[Word]:
        ...
        Returns:
            Optional[Word]: The next word in the queue if available, otherwise None.
        """
    ...
    def check_answer(self, word: Word, user_input: str) -> bool:
        """
        Checks if the user's input matches the foreign term of the given word.

        This method compares the user's input with the foreign term of the word,
        ignoring leading/trailing whitespace and case differences. It updates
        the quiz statistics based on whether the answer is correct or incorrect.

        Args:
            word (Word): The word object containing the foreign term to be checked.
            user_input (str): The user's input to be compared with the foreign term.

        Returns:
            bool: True if the user's input matches the foreign term, False otherwise.
    ...
    def increment_incorrect_repeat(self, word: Word):
        ...
        Args:
            word (Word): The word object containing the foreign term to be incremented.

        Returns:
            int: The updated count of incorrect repetitions for the given foreign term. 
    ...
    def update_words(self, new_words: List[Word]):
        ...
        Args:
            new_words (List[Word]): A list of Word objects to update the internal word list with.
